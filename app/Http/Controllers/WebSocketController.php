...
class WebSocketController extends Controller implements MessageComponentInterface
{
    ...
    function onOpen(ConnectionInterface $conn){
        $this->connections[$conn->resourceId] = compact('conn') + ['user_id' => Str::random(10)];
    }
    ...
    function onMessage(ConnectionInterface $conn, $msg){
        // dd($msg);
        // return 0;
        // dd($this->connections[$conn->resourceId]['user_id']);
        // dd($this->connections);
        if(is_null($this->connections[$conn->resourceId]['user_id'])){
            ...
            // dd($msg['data']);
            // dd($this->connections[0]);
            foreach ($this->connections as $key => $value) {
                $this->connections[$key]['conn']->send(json_encode([
                    'msg' => $msg['data'],
