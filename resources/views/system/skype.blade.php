  ...
  <div class="az-content az-content-dashboard">
      <div class="container">
        <div class="az-content-body">
          ...
          <b><i>JSON data link</i></b>: <a href="{{route('vmix_data_source.get_source_text')}}?table=skype_source_tbl" target="_blank">{{route('vmix_data_source.get_source_text')}}</a>
        </div><!-- az-content-body -->
      </div>
    </div><!-- az-content -->
@endsection
...
$("a#button-save").on('click', function(){
  $.ajax({
    url: "{{route('system.button.save_skype')}}",
    type: "POST",
    data: {
    ...
    success: function(e){
      ...
      conn.send(JSON.stringify({data: {tag: "skype", message: $("textarea#content").val()} }));
    },
    error: function(e){
    }
  });
