  ...
  <div class="az-content az-content-dashboard">
      <div class="container">
        <div class="az-content-body">
          ...
          <b><i>JSON data link</i></b>: <a href="{{route('vmix_data_source.get_source_excel')}}?table=next_and_now_tbl" target="_blank">{{route('vmix_data_source.get_source_excel')}}</a>
        </div><!-- az-content-body -->
      </div>
    </div><!-- az-content -->
@endsection
...
<script>
  $(document).ready(function(){
    var grid;
    $.ajax({
      url: "{{route('system.next_and_now_tbl')}}",
      async: false,
      success: function(e){
  ...
  $("a#button-save").on('click', function(){
    ...
    $.ajax({
      url: "{{route('system.button.save_next_and_now')}}",
      type: "POST",
      data: {
      ...
      success: function(e){
        // console.log(e);
        conn.send(JSON.stringify({data: {cell_position: data,tag: "next_and_now"} }));
        $.toast({
              heading: 'Success',
              text: "Document Saved",
  ...
  $("div#grid").on("keydown", function(){
    conn.send(JSON.stringify({data: {cell_position: null,tag: "next_and_now_typing"} }));
  });
