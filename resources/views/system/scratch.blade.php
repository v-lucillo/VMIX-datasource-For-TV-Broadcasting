  ...
  <div class="az-content az-content-dashboard">
      <div class="container">
        <div class="az-content-body">
          ...
          <b><i>JSON data link</i></b>: <a href="{{route('vmix_data_source.get_source_excel')}}?table=scratch_tbl " target="_blank">{{route('vmix_data_source.get_source_excel')}}</a>
        </div><!-- az-content-body -->
      </div>
    </div><!-- az-content -->
@endsection
...
<script>
  $(document).ready(function(){
    ...
    $("a#button-save").on('click', function(){
      ...
      $.ajax({
        url: "{{route('system.button.save_scratch')}}",
        type: "POST",
        data: {
        ...
        success: function(e){
          conn.send(JSON.stringify({data: {cell_position: data,tag: "scratch"} }));
          $.toast({
                heading: 'Success',
                text: "Document Saved",
    ...
    $("div#grid").on("keydown", function(){
      conn.send(JSON.stringify({data: {cell_position: null,tag: "scratch_typing"} }));
    });
